...
- 收藏夹中的下架 / 失效视频会累计命中次数，达到阈值后自动跳过
- WebUI 将黑名单视频和普通视频一起展示，可按状态筛选，并可直接手动加入 / 移出黑名单
- WebUI 可展开播放各分 P，并支持跳转 B 站页面 / UP 主空间 / 在资源管理器中选中视频或弹幕文件
- 服务不可用时默认不再自动降级到浏览器下载，可在 v4 面板中手动开启
- 智能格式选择
  - 同档优先高帧率
  - 能保住 4K 就不降到 1080P
  - 预计体积超过 `1 GiB` 时自动降档
...
5. 如需在服务未运行时退回浏览器下载，到面板里打开“服务不可用时启用浏览器降级”
6. 打开 `http://127.0.0.1:18888/webui` 查看本地库
