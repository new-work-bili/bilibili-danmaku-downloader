...
- 支持 WebUI
- 支持失效 / 下架视频自动跳过
- 支持服务端持久化黑名单、状态筛选与手动加入 / 恢复
- 支持同名覆盖
- 服务不可用时默认不降级，可按需打开浏览器降级下载弹幕
...
- “服务不可用时启用浏览器降级”默认关闭；只有手动打开后，弹幕和轮询日志才会退回浏览器默认下载目录
- 浏览器降级模式下不保留子文件夹，也不能保证同名覆盖
...
- 自动触发对应视频的全部分 P 下载
- 配置收藏夹 ID 并开启轮询
- 收藏夹中明确失效 / 下架的视频会累计命中次数，达到 `5` 次后加入黑名单
- 黑名单中的视频后续轮询会直接跳过，不再继续触发视频下载
...
- 按 BV 查看本地视频卡片列表
- 显示封面、标题、UP 主、添加收藏时间、弹幕更新时间、视频发布时间与总 P 数
- 按“添加收藏时间 / 更新弹幕时间 / 视频发布时间”排序
- 按 UP 主名字筛选
- 按状态筛选“全部视频 / 仅黑名单 / 排除黑名单”
- 黑名单视频直接混排在主视频流中，并在卡片上显示黑名单标记
- 在卡片或详情弹层中直接把某个 BV 加入黑名单，或从黑名单移出
- 在详情弹层中查看黑名单原因、命中次数、最近来源、首次 / 最近发现时间与最近错误摘要
- 在详情弹层中打开原视频链接和对应 UP 主空间
- 将某个 BV 从黑名单移除后，不会立刻下载；等待下次收藏夹轮询重新尝试
- 通过“查看分P”详情弹层浏览并播放指定分 P
- 跳转到对应视频的 B 站页面
- 点击 UP 主名字跳转到对应空间
- 打开视频所属文件夹并在资源管理器中选中视频文件
- 选中对应分 P 或主卡片关联的弹幕 XML 文件

### 升级后如果功能没有生效

如果一次迭代改到了本地服务、WebUI 或油猴脚本，通常需要你手动做以下动作：

1. 重启 `danmaku-server.mjs`
2. 对 `http://127.0.0.1:18888/webui` 执行一次强制刷新
3. 如果改到了油猴脚本，重新安装或刷新 `src/get-danmaku-v4.user.js`
4. 如果新功能依赖新增元数据，再重新跑一次手动下载或收藏夹轮询
5. 如果需要验证黑名单恢复是否生效，手动再执行一次收藏夹轮询
...
- `state/download-blacklist.json` 保存失效 / 下架视频的累计命中状态与黑名单条目
