...
## 2026-04-19 v4 浏览器降级下载改为可配置

### 背景

`v4` 之前在本地服务未运行或 `/save` 写入失败时，会自动退回浏览器内置下载。这会掩盖服务未启动的问题，也容易让用户误以为弹幕仍然写进了固定目录。

### 本轮目标

- 把浏览器降级下载改成显式开关，而不是默认自动触发
- 默认关闭该开关，避免服务不可用时“看起来成功、实际落到浏览器默认下载目录”
- 同步补充面板提示和使用文档

### 已完成

- `src/get-danmaku-v4.user.js` 新增 `BROWSER_FALLBACK_ENABLED` 持久化配置项
- v4 面板新增“服务不可用时启用浏览器降级”开关，默认关闭
- 服务状态文案改为根据开关实时显示“将降级为浏览器下载”或“浏览器降级已关闭”
- 弹幕文件保存逻辑改为在降级关闭时显式返回失败，不再默默触发浏览器下载
- 手动下载、合并下载、收藏夹轮询和轮询日志写入都已改为正确处理“保存失败”结果
- 轮询与手动下载状态会额外标记哪些文件走了浏览器降级

### 遗留事项

- 当前开关只放在油猴面板里，暂未增加脚本菜单或源码级默认值覆盖入口
- 视频下载仍然依赖本地服务，本轮没有给视频链路增加浏览器侧回退
...
- README 和 usage 已同步改为“浏览器降级默认关闭，可按需开启”
- 本文补记了 v4 从“自动降级”改为“显式配置”的行为调整
