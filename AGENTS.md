...
如果任务涉及“上一轮做了什么”“这次需求是怎么演进来的”“某个能力是什么时候加上的”，读：
...
- 如果一次迭代涉及用户手动操作，必须在最终回复里显式列出
- 典型手动操作包括：
  - 需要重启 `danmaku-server.mjs`
  - 需要强刷 `WebUI`
  - 需要重新安装或刷新 Tampermonkey 脚本
  - 需要重新跑一次下载或收藏夹轮询来补齐旧元数据
- 不要把这些动作埋在长段解释里，必须用清晰、易执行的方式告诉用户
...
- 每轮需求迭代都尽量在 `docs/iteration-log.md` 留痕
